      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div className="flex items-center gap-2 overflow-x-auto pb-1 max-w-full no-scrollbar">
          <button
            ...
            className={`px-4 py-2 rounded-2xl text-xs font-black transition-all cursor-pointer whitespace-nowrap border ${
              selectedCatFilter === 'all'
                ? 'bg-[#1c3a1e] text-white border-[#1c3a1e] shadow-xs'
                : 'bg-white text-gray-700 border-[#1c3a1e]/15 hover:bg-[#eaf2eb]'
            }`}
          >
        ...
        {filteredItems.map((item) => {
          ...
          return (
            <div
              ...
              className={`bg-white border border-[#1c3a1e]/15 hover:border-[#1c3a1e] rounded-2xl p-3 flex flex-col justify-between shadow-xs hover:shadow-md transition-all cursor-pointer group relative overflow-hidden ${
                isAddingThis ? 'ring-2 ring-emerald-500 border-emerald-500' : ''
              }`}
            >
              <div>
                ...
                <h3 className="text-xs font-black text-[#1c3a1e] line-clamp-1 group-hover:text-[#d4af37] transition-colors">
                  {item.name}
                </h3>
              ...
              <div className="flex justify-between items-center mt-2 pt-2 border-t border-[#1c3a1e]/10">
                <span className="font-black text-xs text-[#1c3a1e]">
