...
async function ensureLoyaltyTables() {
  ...
  try {
    ...
    // One-time repair: fix any order_items that were accidentally assigned to closed sessions after session closed
    await pool.query(`
      UPDATE order_items oi
      SET session_id = ts_active.id
      FROM table_sessions ts_closed, table_sessions ts_active
      WHERE oi.session_id = ts_closed.id
        AND ts_closed.status = 'closed'
        AND ts_closed.closed_at IS NOT NULL
        AND oi.created_at > ts_closed.closed_at
        AND ts_active.primary_table_id = ts_closed.primary_table_id
        AND ts_active.status = 'active'
    `).catch(() => {});
  } catch (e) {
    console.error('Error creating loyalty tables:', e);
  }
}
