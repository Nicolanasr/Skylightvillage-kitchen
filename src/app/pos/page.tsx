...
export default function POSPage() {
  ...
  const handleCompleteGuestPayment = async () => {
    ...
    const res = await processSplitPayment({
      ...
      paidItemIds: targetItems.map((i) => ({ id: i.id, qty: 1 })),
    });
  ...
  const handleAddWaiterItemSubmit = async () => {
    if (!selectedTable || !selectedMenuItemForWaiter) return;
  ...
  // SIMPLIFIED FLOOR MATRIX: Filter out secondary merged tables so merged tables appear as 1 combined card
  const visibleTablesOnMatrix = tables.filter((t) => t.status !== 'merged');
      ...
      {readyForDeliveryItems.length > 0 && (
        <div className="mb-6 bg-emerald-500/10 border-2 border-emerald-500/40 rounded-2xl p-4 animate-in fade-in shadow-xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {readyForDeliveryItems.map((item) => (
              <div
                key={item.id}
                className="bg-slate-900 border border-emerald-500/40 rounded-xl p-3 flex items-center justify-between"
              >
                <div>
                ...
                <button
                  ...
                  className="bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-black px-3 py-1.5 rounded-lg text-xs transition-colors flex items-center gap-1 shadow-md"
                >
                  <span>Delivered</span>
            ...
            {visibleTablesOnMatrix.map((tbl) => {
              ...
              return (
                <div
                  ...
                  className={`glass-card rounded-2xl p-4 flex flex-col justify-between cursor-pointer border-2 transition-all min-h-[140px] relative ${statusColorClasses} ${
                ...
                <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
                  {sessionItems.length === 0 ? (
                    ...
                    sessionItems.map((item) => (
                      <div
                        key={item.id}
                        className={`bg-slate-950 border rounded-xl p-3 ${
                          item.is_paid
                        ...
                        <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-900">
                          ...
                          {!item.is_paid && (
                            item.status === 'cancelled' ? (
                              ...
                              <div className="flex items-center gap-3">
                                <button
                                  onClick={() => compOrderItem(item.id, !item.is_comped)}
                ...
                <button
                  onClick={async () => {
                    ...
                    const res = await processSplitPayment({
                      ...
                      paymentMethod,
                      paymentType: 'full',
                      paidItemIds: unpaidItems.map((i) => ({ id: i.id, qty: 1 })),
                    });
                    if (res.success) {
                      setReceiptToPrint({
                        tableNumber: selectedTable?.table_number || 1,
                ...
                {filteredMenuItemsForWaiter.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-3 rounded-xl border text-left transition-all flex justify-between items-start ${
                        isSelected
                ...
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-2xl space-y-3 mb-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Requested by guest, special arrangement..."
